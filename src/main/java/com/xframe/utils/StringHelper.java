...
public final class StringHelper {
	...
	/**
	 * 
	* @Title: objectStrToMap 
	* @Description: TODO(把一个对象（object）字符串转换成map结构) 
	* @param  objectStr : 对象字符串
	* @return Map<String,Object>  对象中所有的 成员变量 和属性名
	* @throws
	 */
	public static Map<String,Object> objectStrToMap(String objectStr){
			if(objectStr == null) {
				return null;
			}
		Map<String,Object> backMap = new HashMap<>();
			// 切割前后字符串
			 String substring = objectStr.substring(1,objectStr.length()-1);
			 
			System.out.println(substring);
			//分割 逗号:得到每个键值对
			String[] split = split(substring, COMMA);
			
			for (String string : split) {
				// 把键值对 分割
				String[] object = split(string, "=");
				// 去除左侧空格
				backMap.put(leftTrim(object[0]),leftTrim(object[1]));
			}
	...
	/**
	 * 把一个 map结构的key 与value 按照指定字符拼接成字符串  按指定字符隔开
	* @Title: mapToString 
	* @Description: 把一个 map结构的key 与value 按照指定字符拼接成字符串  按指定字符隔开
	* @param convertedMap ：要拼接的 map
	* @param joinStr    key与value拼接的字符
	* @param spitStr    隔开的字符
	* @return String    返回类型 
	* @throws
	 */
	public static String mapToString(Map<String,Object> convertedMap,String joinStr ,String spitStr) {
		if(convertedMap == null ||isBlank(joinStr)) {
			return null;
		}
		StringBuilder sb = new StringBuilder();
		for(Map.Entry<String, Object> m:convertedMap.entrySet()) {
			sb.append(m.getKey()+joinStr+m.getValue()+spitStr);
		}	
